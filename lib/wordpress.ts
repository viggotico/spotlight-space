...
import { Post, Category, Tag, Page, Author, FeaturedMedia, type Media } from './wordpress.d';
...
export async function wordpressFetch<T>(url: string, options?: FetchOptions): Promise<T> {
    const headersList = await headers();
    const userAgent = headersList.get('user-agent') || 'Spotlight Space Frontend';
...
export async function getMediaById(id: number): Promise<Media> {
    ...
    const response = await wordpressFetch<Media>(url, {
        next: {
            ...defaultFetchOptions.next,
...
export async function getMedia(query?: FetchQuery): Promise<Media[]> {
    const url = getUrl(`/wp-json/wp/v2/media`, query);
    const response = await wordpressFetch<Media[]>(url, {
        next: {
            ...defaultFetchOptions.next,
            tags: ['wordpress', 'media', JSON.stringify(query ?? {})],
        },
    });
